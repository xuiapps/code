...
/// </summary>
internal sealed unsafe class MetalCommandList : IGpuCommandList
{
    ...
    private MetalVertexShader? _currentVertexShader;
    private MetalFragmentShader? _currentFragmentShader;
    private nint _vertexBuffer;
    private int _vertexCount;
    private nint _constantBuffer;
    private bool _disposed;

    internal MetalCommandList(nint device, nint commandQueue)
    {
        _device = device;
    ...
    public void SetPipeline(GpuPipelineDesc pipeline)
    {
        ...
        _currentVertexShader = pipeline.VertexShader as MetalVertexShader;
        _currentFragmentShader = pipeline.FragmentShader as MetalFragmentShader;

        if (_currentVertexShader == null)
            throw new InvalidOperationException("Pipeline requires a MetalVertexShader.");
        ...
        if (_pipelineState != 0)
        {
            MetalNative.Release(_pipelineState);
            _pipelineState = 0;
        }

        _pipelineState = MetalNative.Device_NewRenderPipelineState(
            _device, pipelineDesc, out nint error);

        MetalNative.Release(pipelineDesc);
    ...
    public void SetConstantBuffer(void* data, int sizeInBytes)
    {
        ...
        {
            MetalNative.Release(_constantBuffer);
            _constantBuffer = 0;
        }
        ...
        if (data != null && sizeInBytes > 0)
        {
            ...
            _constantBuffer = MetalNative.Device_NewBufferWithBytes(
            ...
            MetalNative.Encoder_SetVertexBuffer(_encoder, _constantBuffer, 0, 1);
        }
    }
    ...
    public void Draw(int vertexCount)
    {
        ...
        MetalNative.Encoder_DrawPrimitives(_encoder, 3, 0, (ulong)vertexCount);
    }
    ...
    public void Commit()
    {
        ...
        MetalNative.CommandBuffer_Commit(_commandBuffer);
        MetalNative.CommandBuffer_WaitUntilCompleted(_commandBuffer);

        // Release command buffer after completion
        MetalNative.Release(_commandBuffer);
        _commandBuffer = 0;
    }
    ...
    public void Dispose()
    {
        ...
        if (_commandBuffer != 0) MetalNative.Release(_commandBuffer);
    }
}
